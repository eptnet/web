...
serve(async (req) => {
  ...
  try {
    ...
    if (likeUri) {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Like eliminado' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
    } else {
      ...
      const likeRecord = { $type: 'app.bsky.feed.like', subject: { uri: postUri, cid: postCid }, createdAt: new Date().toISOString() };
      const createBody = { repo: creds.did, collection: 'app.bsky.feed.like', record: likeRecord };
      ...
      return new Response(JSON.stringify({ success: true, message: 'Post likeado', uri: likeResult.uri }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
    }

  } catch (error) {
    console.error("Error en like-post:", error.message);
