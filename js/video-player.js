/**
 * =========================================================================
 * ShortsPlayerManager - VERSIÓN FINAL v14.2 (Integrada con el Historial)
 * - Se integra con el sistema de historial de app.js.
 * - Mantiene toda la lógica original de carga y caché.
 * =========================================================================
 */
// *** CAMBIO CLAVE: Esperamos el evento 'mainReady' para asegurar que Supabase esté listo.
document.addEventListener('mainReady', () => {
    ...
    const ShortsPlayerManager = {
        ...
        init() {
            ...
            // Escucha la orden de cierre que viene del "jefe" (app.js popstate)
        ...
        loadYouTubeAPI() {
            ...
            window.onYouTubeIframeAPIReady = () => {
                ...
                // Si la UI ya se construyó mientras la API cargaba, inicializa el player.
        ...
        async launch() {
            ...
            // *** CAMBIO CLAVE: Registramos en el historial ANTES de hacer cualquier otra cosa.
            history.pushState({ storiesOpen: true }, '');
        ...
        setupPanel() {
            const sidePanel = document.getElementById('side-panel');
            // Ocultamos la barra de compartir para dar espacio al reproductor
            ...
            // Ya no necesitamos este evento, el control lo tiene el historial
            // document.dispatchEvent(new CustomEvent('panel-opened'));
        },
        
        // El resto de tus funciones (buildUI, initMainPlayer, etc.) se mantienen sin cambios
        buildUI() {
            const uiHTML = `<div id="main-player-container"></div><button id="player-mute-btn" title="Activar/desactivar sonido"><i class="fa-solid fa-volume-high"></i></button><div id="thumbnail-carousel-container"><button id="carousel-toggle-btn" title="Mostrar/ocultar miniaturas"><i class="fa-solid fa-chevron-down"></i></button><div class="swiper"><div class="swiper-wrapper">${this.videos.map((video,index)=>`<div class="swiper-slide thumbnail-slide" data-video-id="${video.id}" data-index="${index}"><img src="${video.thumbUrl}" alt="miniatura"/></div>`).join('')}</div></div></div>`;
        ...
        initThumbnailSwiper() {
            this.swiper = new Swiper('#thumbnail-carousel-container .swiper', {
                slidesPerView: 3.5, spaceBetween: 10, centeredSlides: true, loop: false, slideToClickedSlide: false,
            });
        },
        ...
        addEventListeners() {
            ...
            this.swiper?.on('slideChange', (swiper) => { this.updateActiveThumbnail(swiper.realIndex); if (swiper.activeIndex >= this.videos.length - 3) this.loadMoreVideos(); });
        },
        ...
        toggleMute() { if (!this.mainPlayer) return; const muteBtn = document.getElementById('player-mute-btn'); const icon = muteBtn?.querySelector('i'); if (this.mainPlayer.isMuted()) { this.mainPlayer.unMute(); if(icon) icon.className = 'fa-solid fa-volume-high'; } else { this.mainPlayer.mute(); if(icon) icon.className = 'fa-solid fa-volume-xmark'; } },
        async loadMoreVideos() { if (!this.nextPageToken || this.isLoadingMore) return; this.isLoadingMore = true; try { const searchQuery = encodeURIComponent('"#Shorts" | "#Short"'); const apiUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${this.CHANNEL_ID}&maxResults=15&q=${searchQuery}&type=video&order=date&videoDuration=short&key=${this.YOUTUBE_API_KEY}&pageToken=${this.nextPageToken}`; const response = await fetch(apiUrl); if (!response.ok) throw new Error('No se pudo cargar más.'); const data = await response.json(); this.nextPageToken = data.nextPageToken; const newVideos = data.items.map(item => ({ id: item.id.videoId, thumbUrl: item.snippet.thumbnails.high.url })); const newSlidesHTML = newVideos.map((video, index) => `<div class="swiper-slide thumbnail-slide" data-video-id="${video.id}" data-index="${this.videos.length + index}"><img src="${video.thumbUrl}" alt="miniatura"/></div>`).join(''); this.swiper.appendSlide(newSlidesHTML); this.videos.push(...newVideos); this.setCache(this.videos, this.nextPageToken); } catch (error) { console.error("Error cargando más videos:", error); } finally { this.isLoadingMore = false; } },
        ...
        destroy() {
            ...
            // La parte visual de cerrar el panel (overlay, body.overflow)
            // ahora la gestiona el popstate en app.js
        },
    };
