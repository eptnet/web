/**
 * Script Principal para Epistecnologia.com
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openSidePanel(cardElement) {
        ...
        let contentHTML = '';
        let shareableData = { link: window.location.href, title: 'Epistecnologia' };

        if (dataset.panelType === 'embed' && dataset.embedSrc) {
            const title = dataset.panelTitle || 'Contenido Adicional';
            shareableData = { link: dataset.embedSrc, title: title };
            contentHTML = `
                <h2>${title}</h2>
                <div class="post-body">
            ...
            if (!postData) return;
            shareableData = { link: postData.link, title: postData.title };
        ...
        cleanupPostContent();
        setupShareButtons(shareableData);
    ...
    function closeSidePanel() {
        ...
        // Buscamos si hay un iframe dentro del panel.
        const iframe = sidePanelContent.querySelector('iframe');
        const audio = sidePanelContent.querySelector('audio');

        // Si se encuentra un iframe (video), se resetea su 'src' para detener la reproducción.
        if (iframe) {
            iframe.src = iframe.src; // Una forma de forzar la recarga y detener el video.
        }
        // Si se encuentra un audio, lo pausamos.
        if (audio) {
            audio.pause();
        }
    ...
    // 5. FUNCIONES DE RENDERIZADO ESPECÍFICO
