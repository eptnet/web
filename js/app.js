/**
 * =========================================================================
 * Script Principal para la Página de Inicio - Versión Final Estable 13.0
 * =========================================================================
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupShareButtons(config) {
        ...
        document.querySelector("#share-fb")?.addEventListener('click',()=>window.open(`https://www.facebook.com/sharer/sharer.php?u=${link}`));
        document.querySelector("#share-li")?.addEventListener('click',()=>window.open(`https://www.linkedin.com/shareArticle?mini=true&url=${link}&title=${title}`));
        document.querySelector("#share-wa")?.addEventListener('click',()=>window.open(`https://api.whatsapp.com/send?text=${title}%20${link}`));
        document.querySelector("#share-x")?.addEventListener('click',()=>window.open(`https://twitter.com/intent/tweet?url=${link}&text=${title}`));
        const copyBtn = document.querySelector("#copy-link");
        if(copyBtn) copyBtn.onclick=()=>{navigator.clipboard.writeText(config.link).then(()=>{copyBtn.innerHTML='<i class="fa-solid fa-check"></i>';setTimeout(()=>{copyBtn.innerHTML='<i class="fa-solid fa-link"></i>'},2e3)}).catch(e=>console.error("Error al copiar",e))};
    }
    ...
    function initHero() {
        ...
        if (heroButton && desktopNav) {
            heroButton.addEventListener('click', () => {
                const scrollTarget = window.innerWidth > 768 ? desktopNav : document.querySelector('main.container');
        ...
        if (mobileNav) {
             const checkMobileNavPosition = () => {
                mobileNav.classList.toggle('is-visible', window.scrollY > 50);
            };
            window.addEventListener('scroll', checkMobileNavPosition, { passive: true });
    ...
    function initAuthAndTheme() {
        ...
        const applyTheme = (theme) => {
            ...
            themeSwitcherDesktop?.querySelector('i')?.setAttribute('class', `fa-solid ${iconClass}`);
            themeSwitcherMobile?.querySelector('i')?.setAttribute('class', `fa-solid ${iconClass}`);
        };
        const toggleTheme = () => {
        ...
        const showUserUI = (user) => { if(guestView) guestView.style.display = 'none'; if(userView) userView.style.display = 'flex'; if(avatarLink && user) { const avatarUrl = user.user_metadata?.avatar_url || '/img/default-avatar.png'; avatarLink.innerHTML = `<img src="${avatarUrl}" alt="Avatar" style="width: 32px; height: 32px; border-radius: 50%;">`; }};
        const showGuestUI = () => { if(userView) userView.style.display = 'none'; if(guestView) guestView.style.display = 'flex'; };
        const handleOAuthLogin = async (provider) => { await window.supabaseClient.auth.signInWithOAuth({ provider, options: { redirectTo: `${window.location.origin}/inv/dashboard.html` } }); };

        window.supabaseClient.auth.onAuthStateChange((_event, session) => { session?.user ? showUserUI(session.user) : showGuestUI(); if (_event === 'SIGNED_IN') window.location.href = '/inv/dashboard.html'; });
        async function checkInitialSession() { const { data: { session } } = await window.supabaseClient.auth.getSession(); session?.user ? showUserUI(session.user) : showGuestUI(); }
        checkInitialSession();
