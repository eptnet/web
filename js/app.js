/**
 * =========================================================================
 * Script de la Página de Inicio (app.js) - VERSIÓN COMPLETA Y FINAL
 * - Soluciona el problema de reproducción de MP3s usando el elemento <audio> nativo.
 * - Mantiene toda la lógica del Modal de Inmersión y la carga de posts.
 * =========================================================================
 */
...
document.addEventListener('mainReady', () => {
    ...
    function openModal(content, type = 'article', shareConfig = null, audioSrc = null) {
        ...
        if (type === 'stories') {
            ...
            if (audioSrc) {
                ...
                audioElement.id = 'native-audio-player';
                audioElement.crossOrigin = 'anonymous'; // Necesario para que wavesurfer pueda analizar el audio
                playerContainer.appendChild(audioElement);

                if (typeof WaveSurfer !== 'undefined') {
                    window.wavesurferInstance = WaveSurfer.create({
                        container: '#waveform',
    ...
    bentoGrid?.addEventListener("click", (event) => {
        ...
        if (dataId === "static-launch-stories") {
            ...
            const audioSrc = post.enclosure?.link?.endsWith(".mp3") ? post.enclosure.link : null;
            
            openModal(contentHTML, 'article', { link: post.link, title: post.title }, audioSrc);
        }
    });
