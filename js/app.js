 ...
 * Versión Final: Utiliza URLs de embed completas para máxima compatibilidad.
 * =========================================================================
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- PLANTILLAS DE MÓDULOS CON URLs COMPLETAS ---
    ...
    // Tarjeta de Historias con nueva imagen de fondo
    const videoStoriesCardHTML = `
        <div class="bento-box bento-box--1x3" data-id="static-launch-stories" style="background-image: url('https://i.postimg.cc/NMyQ6Hd1/stories-bg.jpg'); cursor: pointer; background-size: cover; background-position: center;">
    ...
    // Video Destacado con URL de embed completa
    const videoFeaturedModuleHTML = `
        <div class="bento-box bento-box--2x2 video-featured-module" data-id="static-video-featured">
             <div data-plyr-provider="youtube" data-plyr-embed-id="https://www.youtube.com/embed/2Vq_N_wgUkk"></div>
        </div>
    `;
    ...
    // (El resto del archivo 'app.js' es idéntico y no necesita cambios)
    ...
    function openSidePanel(t){const e=t.dataset;if("static-video-featured"===e.id||"static-launch-stories"===e.id)return;let o="",n=window.location.href;"embed"===e.panelType&&e.embedSrc?(o=`<h2>${e.panelTitle||"Contenido Adicional"}</h2><div class="post-body"><div class="iframe-container"><iframe src="${e.embedSrc}" title="${e.panelTitle||"Contenido Adicional"}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div></div>`,n=e.embedSrc):(()=>{const t=allPostsData.find(t=>t.guid===e.id);if(t){n=t.link;const e=new Date(t.pubDate).toLocaleDateString("es-ES",{year:"numeric",month:"long",day:"numeric"});let i="";t.enclosure?.link?.endsWith(".mp3")&&(i=`<audio controls controlsList="nodownload" src="${t.enclosure.link}"></audio>`),o=`<h2>${t.title}</h2><div class="post-meta">Publicado por ${t.author} el ${e}</div>${i}<div class="post-body">${t.content}</div>`}})()
    sidePanelContent.innerHTML=o,cleanupPostContent(),setupShareButtons({link:n}),sidePanel.classList.add("is-open"),overlay.classList.add("is-open"),document.body.style.overflow="hidden"}
