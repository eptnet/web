/**
 * =========================================================================
 * Script de la Página de Inicio (app.js) - VERSIÓN REORGANIZADA
 * - El layout de la Bento Grid ahora se controla desde el array `grid_layout`.
 * - Mover elementos en `grid_layout` cambia el orden en la web.
 * - Incluye ejemplos de cómo usar las nuevas clases de estilo.
 * =========================================================================
 */
...
document.addEventListener('mainReady', () => {
    ...
    // Aquí están todos tus bentos estáticos en un solo lugar.
    const staticModules = {
        ...
        // EJEMPLO: Aplicando el estilo 'flat' al módulo de la cita
    ...
    // ¡ESTE ES EL ÚNICO LUGAR QUE NECESITAS EDITAR PARA CAMBIAR EL ORDEN!
    ...
    const grid_layout = [
        ...
        { type: 'post' }, // Post 5 (índice 4 del feed)
        { type: 'post' }, // Post 6 (índice 5 del feed)
        { type: 'post' }, // Post 7 (índice 6 del feed)
        { type: 'module', id: 'inFeed' }, // Módulo estático insertado
        { type: 'post' }, // Post 8 (índice 7 del feed)
        { type: 'post' }, // Post 9 (índice 8 del feed)
        { type: 'post' }, // Post 10 (índice 9 del feed)
        { type: 'post' }, // Post 11 (índice 10 del feed)
        { type: 'post' }, // Post 12 (índice 11 del feed)
        { type: 'post' }, // Post 13 (índice 12 del feed)
        { type: 'module', id: 'end' }
    ];
    ...
    // Construye la grid basándose en el "plano" (grid_layout)
    function displayPosts(items) {
        bentoGrid.innerHTML = "";
        let postIndex = 0; // Un contador para saber qué post del feed toca
        ...
        grid_layout.forEach(element => {
            ...
            if (element.type === 'module') {
                ...
                if (post) {
                    ...
                    postIndex++; // Avanzamos al siguiente post para la próxima iteración
                }
            }
            
            if (elementHTML) {
                bentoGrid.insertAdjacentHTML("beforeend", elementHTML);
            }
        });
    }

    // Crea el HTML para una tarjeta de post individual
    function createPostCardHTML(item, index) {
        ...
        // Lógica de tamaño original
        let cardSizeClass = "bento-box--1x1";
        if (index === 0) cardSizeClass = "bento-box--2x2";
        else if (index % 3 === 2) cardSizeClass = "bento-box--1x2";

        // EJEMPLO: Aplicando el estilo 'circle' al 5to post del feed (índice 4)
        if (index === 4) {
             // Para que el círculo se vea bien, forzamos que sea 1x1
            cardSizeClass = "bento-box--1x1 bento-style--circle";
        }
