 ...
 * Versión 10.0 - COMPLETA Y SIN ABREVIATURAS
 * =========================================================================
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyTheme(theme) {
        ...
        document.getElementById('theme-switcher-desktop')?.querySelector('i')?.classList.remove('fa-moon', 'fa-sun');
        document.getElementById('theme-switcher-desktop')?.querySelector('i')?.classList.add('fa-solid', iconClass);
        document.getElementById('theme-switcher-mobile')?.querySelector('i')?.classList.remove('fa-moon', 'fa-sun');
        document.getElementById('theme-switcher-mobile')?.querySelector('i')?.classList.add('fa-solid', iconClass);
    }
    ...
    function showUserUI(user) {
        ...
        if (userView && guestView && avatarLink) {
            guestView.style.display = 'none';
            userView.style.display = 'flex';
            const avatarUrl = user.user_metadata?.avatar_url || 'img/default-avatar.png';
            const userName = user.user_metadata?.full_name || user.email;
            avatarLink.innerHTML = `<img src="${avatarUrl}" alt="Avatar de ${userName}" style="width: 32px; height: 32px; border-radius: 50%;">`;
            logoutBtn?.addEventListener('click', async () => { await supabaseClient.auth.signOut(); }, { once: true });
        }
    }
    ...
    supabaseClient.auth.onAuthStateChange((event, session) => {
        if (session?.user) {
            ...
            if (event === 'SIGNED_IN' && !window.location.pathname.includes('/inv/')) {
    ...
    async function handleOAuthLogin(provider) {
        const { error } = await supabaseClient.auth.signInWithOAuth({ provider, options: { redirectTo: window.location.origin + '/inv/dashboard.html' } });
