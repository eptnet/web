...
const ProfileApp = {
    ...
    renderIdentityPassport(profile, isEditable) {
        if (!isEditable) return; 
        
        const hasOrcid = profile.orcid && profile.orcid !== '0000';
        const hasBsky = !!this.bskyCreds;
        let score = 34; // Cuenta base Google (34%)
        
        // 1. Tarjeta ORCID
        const orcidItem = document.getElementById('item-orcid');
        if (orcidItem && hasOrcid) {
            score += 33;
            orcidItem.className = 'passport-row verified';
            orcidItem.innerHTML = `
                <div class="row-icon"><i class="fa-brands fa-orcid"></i></div>
                <div class="row-info">
                    <strong>Validación Académica</strong>
                    <span class="status-tag">${profile.orcid.replace('https://orcid.org/', '')}</span>
                </div>
                <div class="row-action"><i class="fa-solid fa-circle-check"></i></div>
            `;
            // Clic para desconectar si ya está verificado
            orcidItem.onclick = () => {
                if(confirm('¿Deseas gestionar o desconectar tu cuenta de ORCID?')) this.handleOrcidDisconnect();
            };
        }

        // 2. Tarjeta Bluesky
        const bskyItem = document.getElementById('item-bluesky');
        if (bskyItem && hasBsky) {
            score += 33; 
            bskyItem.className = 'passport-row verified';
            bskyItem.innerHTML = `
                <div class="row-icon"><i class="fa-brands fa-bluesky"></i></div>
                <div class="row-info">
                    <strong>Identidad en Red</strong>
                    <span class="status-tag">@${this.bskyCreds.handle}</span>
                </div>
                <div class="row-action"><i class="fa-solid fa-circle-check"></i></div>
            `;
            bskyItem.onclick = () => {
                if(confirm('¿Deseas desconectar tu cuenta de Bluesky? (Perderás 70 XP)')) this.handleBlueskyDisconnect();
            };
        }

        // 3. Actualizar la Barra de Progreso
        const progressEl = document.getElementById('identity-progress');
        if (progressEl) {
            progressEl.textContent = `${score}%`;
            if (score === 100) {
                progressEl.style.background = 'var(--verified-bg)';
                progressEl.style.color = 'var(--verified-text)';
            }
        }
    },
    ...
    addEventListeners() {
        ...
        if (button.classList.contains('edit-btn')) {
            // FIX: Ahora entiende tanto .bento-card (V2) como .bento-box (V1)
            const bentoBox = button.closest('.bento-card') || button.closest('.bento-box');
            if (bentoBox) this.toggleEditMode(bentoBox, true);
            return;
        }
    ...
    toggleEditMode(bentoBox, isEditing) {
        if (!bentoBox) return; // FIX: Evita el crasheo si no encuentra la tarjeta
    ...
    async renderProfileData(profileId, preloadedProfile = null) {
        ...
        try {
            ...
            // Llamamos al nuevo Widget Pasaporte B2B
            if (isEditable) {
                this.renderIdentityPassport(profile, isEditable);
            }
            
            // FIX: Protegemos contra null si el contenedor no existe en V2
            ...
            // Llamamos al nuevo Widget Pasaporte B2B
            if (isEditable && typeof this.renderIdentityPassport === 'function') {
                this.renderIdentityPassport(profile, isEditable);
            }

            // --- LÓGICA V1 PROTEGIDA ---
            if (typeof this.renderOrcidSection === 'function') this.renderOrcidSection(profile, isEditable);
            if (typeof this.renderSidebarButtons === 'function') this.renderSidebarButtons(profile, isMyOwnProfile);

            if (isMyOwnProfile && typeof this.renderCommunityActionPanel === 'function') {
                this.renderCommunityActionPanel(this.bskyCreds);
            }

            // Ocultar modo edición al cargar (usando la clase V2 o V1)
            const profileForm = document.getElementById('profile-form');
            const platformsForm = document.getElementById('platforms-form');
            if (profileForm) this.toggleEditMode(profileForm.closest('.bento-card') || profileForm.closest('.bento-box'), false);
            if (platformsForm) this.toggleEditMode(platformsForm.closest('.bento-card') || platformsForm.closest('.bento-box'), false);
            ...
            if (isEditable) {
                // FIX: Poblar MODO EDICIÓN de forma segura (Ignora si falta un input)
                const safeSetValue = (id, val) => {
                    const el = document.getElementById(id);
                    if (el) el.value = val || '';
                };

                safeSetValue('display-name', profile.display_name);
                safeSetValue('bio', profile.bio);
                safeSetValue('zenodo-authors', profile.display_name);
                safeSetValue('substack-author-name', profile.substack_author_name);
                safeSetValue('avatar-url', profile.avatar_url);
                safeSetValue('youtube-url', profile.youtube_url);
                safeSetValue('substack-url', profile.substack_url);
                safeSetValue('bsky-url', profile.bsky_url);
                safeSetValue('website-url', profile.website_url);
                safeSetValue('x-url', profile.x_url);
                safeSetValue('linkedin-url', profile.linkedin_url);
                safeSetValue('instagram-url', profile.instagram_url);
                safeSetValue('facebook-url', profile.facebook_url);
                safeSetValue('tiktok-url', profile.tiktok_url);
                
                // Renderizado Premium de Redes Sociales (Pills)
                const platformsView = document.getElementById('view-platforms-list');
                const platforms = [
                    { url: profile.youtube_url, icon: 'fa-youtube', name: 'YouTube' },
                    { url: profile.substack_url, icon: 'fa-newspaper', name: 'Substack' },
                    { url: profile.bsky_url, icon: 'fa-bluesky', name: 'Bluesky' },
                    { url: profile.website_url, icon: 'fa-globe', name: 'Web', isSolid: true },
                    { url: profile.x_url, icon: 'fa-x-twitter', name: 'X' },
                    { url: profile.linkedin_url, icon: 'fa-linkedin', name: 'LinkedIn' },
                    { url: profile.instagram_url, icon: 'fa-instagram', name: 'Instagram' }
                ];

                const activePlatformsHTML = platforms.filter(p => p.url).map(p => {
                    const iconClass = p.isSolid ? 'fa-solid' : 'fa-brands';
                    return `
                        <a href="${p.url}" target="_blank" class="platform-pill" title="${p.name}">
                            <i class="${iconClass} ${p.icon}"></i> <span>${p.name}</span>
                        </a>`;
                }).join('');
                
                if (platformsView) {
                    platformsView.innerHTML = activePlatformsHTML || '<p class="text-muted">No hay redes conectadas.</p>';
                }
    ...
    // --- DENTRO DE ProfileApp ---
        ...
        try {
            ...
            // 2. Actualizar contadores globales (Stats)
            ...
            // Si existen los contadores de la pestaña Actividad
            ...
            // 3. Renderizar Lista de Proyectos (La Tabla)
            const projList = document.getElementById('manage-projects-list');
            if (projList) {
                if (projects && projects.length > 0) {
                    projList.innerHTML = projects.map(p => {
                        const isPublished = (p.doi || p.microsite_is_public);
                        const statusBadge = isPublished ? 
                            '<span class="badge-status badge-published">Publicado</span>' : 
                            '<span class="badge-status badge-draft">Borrador</span>';
                        ...
                        <li class="manage-item" id="manage-proj-${p.id}">
                            <div class="manage-item-info">
                                <p class="manage-item-title">${p.title || 'Proyecto sin título'}</p>
                                <div class="manage-item-meta">
                                    ${statusBadge}
                                    <span><i class="fa-regular fa-calendar"></i> ${date}</span>
                                    ${p.doi ? `<span><i class="fa-solid fa-fingerprint"></i> ${p.doi}</span>` : ''}
                                </div>
                            </div>
                            <button class="btn-delete" onclick="deleteManagementItem('projects', '${p.id}')"><i class="fa-solid fa-trash"></i></button>
                        </li>`;
                    }).join('');
                } else {
                    projList.innerHTML = '<div class="empty-msg">No has creado proyectos aún.</div>';
                }
            }

            // 4. Renderizar Lista de Eventos (La Tabla)
                ...
                if (events && events.length > 0) {
                    eventList.innerHTML = events.map(e => {
                        ...
                        <li class="manage-item" id="manage-event-${e.id}">
                            <div class="manage-item-info">
                                <p class="manage-item-title">${e.session_title}</p>
                                <div class="manage-item-meta">
                                    <span class="event-stats-badge"><i class="fa-solid fa-eye"></i> ${e.peak_viewers || 0}</span>
                                    <span><i class="fa-regular fa-calendar"></i> ${date}</span>
                                </div>
                            </div>
                            <button class="btn-delete" onclick="deleteManagementItem('sessions', '${e.id}')"><i class="fa-solid fa-trash"></i></button>
                        </li>`;
                    }).join('');
                } else {
                    eventList.innerHTML = '<div class="empty-msg">No hay eventos agendados.</div>';
                }
            }
        } catch (error) { 
            console.error("Error cargando tablas de mando:", error); 
        }
    },
};
