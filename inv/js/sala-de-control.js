// =================================================================
// ARCHIVO: /inv/js/sala-de-control.js (Dashboard Grid 100%)
// =================================================================
...
const ControlRoom = {
    ...
    async fetchAndLoadSession() {
        ...
        // Lógicas de Enlaces (CORREGIDAS)
        const directorUrl = session.director_url || '';
        const guestUrl = session.guest_url || ''; // <-- ¡CORREGIDO! Este enlace sí te pedirá permisos de cámara
        const obsUrl = session.viewer_url || '';  // <-- ¡CORREGIDO! Este es el enlace limpio sin botones para OBS

        // --- ACORDEÓN DE PRODUCCIÓN Y OBS ---
        const prodToggleBtn = document.getElementById('produccion-toggle-btn');
        const prodContent = document.getElementById('produccion-collapsible-content');
        const prodChevron = document.getElementById('produccion-chevron');
        const prodBadge = document.getElementById('produccion-badge');

        if (prodToggleBtn) {
            prodToggleBtn.onclick = () => {
                const isHidden = prodContent.classList.toggle('hidden');
                prodChevron.style.transform = isHidden ? 'rotate(-90deg)' : 'rotate(0deg)';
                if (prodBadge) {
                    prodBadge.innerHTML = isHidden ? 'Oculto' : 'Visible';
                    prodBadge.style.color = isHidden ? '#38bdf8' : '#10b981';
                    prodBadge.style.borderColor = isHidden ? '#38bdf8' : '#10b981';
                    prodBadge.style.background = isHidden ? 'rgba(56, 189, 248, 0.1)' : 'rgba(16, 185, 129, 0.1)';
                }
            };
        }
        ...
        this.setupCopyButton('btn-copy-director', directorUrl, "No disponible");
        this.setupCopyButton('btn-copy-guest', guestUrl, "No disponible");
        this.setupCopyButton('btn-copy-obs', obsUrl, "No disponible");

        // --- GUÍA OBS ---
        const btnObs = document.getElementById('btn-download-obs');
        if (btnObs) btnObs.onclick = () => this.generateOBSCollection(session, obsUrl);

        const step3 = document.getElementById('obs-step-3');
        if (step3) {
            const realStreamKey = session.stream_key || 'No asignada';
            let rtmpUrl = session.platform === 'twitch' ? 'rtmp://live.twitch.tv/app' : 'rtmp://a.rtmp.youtube.com/live2';
            if (session.platform === 'vdo_ninja') rtmpUrl = 'Servidor Interno EPT';

            step3.innerHTML = `
                Pega en OBS (Ajustes > Emisión):<br>
                <div style="margin-top:5px; background:#000; padding:8px; border-radius:4px; font-family:monospace; border:1px dashed #444;">
                    <strong>Servidor:</strong> ${rtmpUrl}<br>
                    <strong>Clave:</strong> <span style="filter:blur(4px); cursor:pointer;" onclick="this.style.filter='none'" title="Revelar">${realStreamKey}</span>
                </div>
            `;
        }
    ...
    updateStreamControlsUI(status) {
        ...
        if (status === 'PROGRAMADO' || !status) {
            badge.classList.add('status-waiting'); badge.innerHTML = '<i class="fa-solid fa-clock"></i> En Espera';
            btnAction.className = 'btn-start-stream'; btnAction.innerHTML = '<i class="fa-solid fa-play"></i> Iniciar Transmisión Pública';
            btnAction.disabled = false; btnAction.onclick = () => this.startBroadcast();
        } else if (status === 'EN VIVO' || status === 'EN_VIVO') {
            badge.classList.add('status-live'); badge.innerHTML = '<i class="fa-solid fa-tower-broadcast"></i> EN VIVO';
            btnAction.className = 'btn-stop-stream'; btnAction.innerHTML = '<i class="fa-solid fa-stop"></i> Finalizar Transmisión';
            btnAction.disabled = false; btnAction.onclick = () => this.stopBroadcast();
        } else if (status === 'FINALIZADO') {
            badge.classList.add('status-ended'); badge.innerHTML = '<i class="fa-solid fa-flag-checkered"></i> Evento Finalizado';
            btnAction.className = 'btn-ended'; btnAction.innerHTML = '<i class="fa-solid fa-lock"></i> Transmisión Cerrada';
            btnAction.disabled = true; btnAction.onclick = null;
        }
    },

    async startBroadcast() {
        if (!confirm("¿Seguro que deseas INICIAR la transmisión?")) return;
        const btn = document.getElementById('btn-stream-action');
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Iniciando...'; btn.disabled = true;

        const { error } = await this.supabase.from('sessions').update({ status: 'EN VIVO' }).eq('id', this.sessionId);
        if (error) { alert("Error: " + error.message); this.updateStreamControlsUI('PROGRAMADO'); return; }

        this.sessionData.status = 'EN VIVO'; this.updateStreamControlsUI('EN VIVO');
    ...
    async stopBroadcast() {
        ...
        const btn = document.getElementById('btn-stream-action');
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Finalizando...'; btn.disabled = true;
        ...
        try {
            ...
            this.sessionData.status = 'FINALIZADO'; this.updateStreamControlsUI('FINALIZADO');
            if (this.realtimeChannel) this.realtimeChannel.send({ type: 'broadcast', event: 'stream_status_changed', payload: { status: 'FINALIZADO' } });
        } catch (err) {
            alert("Error al finalizar: " + err.message); this.updateStreamControlsUI('EN VIVO');
        }
    },

    async setupRealtimeChat() {
        const { data: pastMessages } = await this.supabase.from('live_chat_messages').select('*').eq('session_id', this.sessionId).order('created_at', { ascending: true }).limit(100);
        if (pastMessages) pastMessages.forEach(msg => this.renderIncomingMessage(msg));

        this.realtimeChannel = this.supabase.channel(`room_${this.sessionId}`, { config: { presence: { key: this.userProfile?.id || 'director' } } });
        this.realtimeChannel
            ...
            .on('broadcast', { event: 'new_chat_message' }, () => this.loadBlueskyComments())
    ...
    renderIncomingMessage(msg) {
        ...
        const modControls = `<div style="margin-left: auto; display:flex; gap: 10px;">
                <button onclick="ControlRoom.promoteToBluesky('${msg.id}', '${msg.user_name}', \`${msg.message.replace(/'/g, "\\'")}\`, '${msg.video_timestamp || 'En Vivo'}')" title="Enviar a Bluesky" style="background:transparent; border:none; color:#38bdf8; cursor:pointer; font-size:0.95rem;"><i class="fa-brands fa-bluesky"></i></button>
                <button onclick="ControlRoom.deleteChatMessage('${msg.id}')" title="Borrar mensaje" style="background:transparent; border:none; color:#ef4444; cursor:pointer; font-size:0.95rem;"><i class="fa-solid fa-trash"></i></button>
            </div>`;
        feed.insertAdjacentHTML('beforeend', `<div id="ctrl-msg-${msg.id}" class="chat-msg" style="display:flex; flex-direction:column;">
                <div class="chat-msg-header" style="display:flex; width:100%; align-items:center;">
                    <span class="chat-msg-author ${isDir}">${msg.user_name}</span><span class="chat-msg-time">${timeStr}</span>${modControls}
                </div><p class="chat-msg-text" style="margin-top:5px;">${msg.message}</p>
            </div>`);
    ...
    async deleteChatMessage(messageId) {
        ...
        try { await this.supabase.from('live_chat_messages').delete().eq('id', messageId); } catch (e) { console.error(e); }
    },

    async promoteToBluesky(messageId, authorName, text, timestamp) {
        if (!confirm(`¿Enviar aporte a Bluesky?`)) return;
        ...
        try {
            ...
            const formattedText = `🎙️: "${text}"\n— ${shortName.substring(0, 10)}${timeBadge}`;
            const { error } = await this.supabase.functions.invoke('bsky-create-reply', { body: { replyText: formattedText, parentPost: { uri: this.sessionData.bsky_chat_thread_uri, cid: this.sessionData.bsky_chat_thread_cid } } });
            if (error) throw error;
            if(btn) { btn.className = 'fa-solid fa-check'; btn.style.color = '#10b981'; }
        } catch (error) { if(btn) btn.className = 'fa-brands fa-bluesky'; alert("Error Bluesky."); }
    },
    ...
    async loadBlueskyComments() {
        ...
        if (!feed || !this.sessionData.bsky_chat_thread_uri) {
            if(feed) feed.innerHTML = '<p class="text-muted text-center" style="margin-top:20px;">No hay hilo vinculado.</p>';
            return;
        }
        try {
            const { data: chatData, error } = await this.supabase.functions.invoke('bsky-get-post-thread', { body: { postUri: this.sessionData.bsky_chat_thread_uri } });
            if (error) throw error;
            feed.innerHTML = '';

            const renderMsg = (msg, isAnchor) => {
                if(!msg || !msg.author) return;
                const record = msg.record || {}; const text = record.text || '';
                const dateStr = (record.createdAt || msg.indexedAt) ? new Date(record.createdAt || msg.indexedAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) : '';
                ...
                feed.insertAdjacentHTML('beforeend', `<div class="bsky-comment" style="${anchorStyle}">
            ...
            if (chatData.anchorPost) renderMsg(chatData.anchorPost, true);
            if (!chatData.messages || chatData.messages.length === 0) {
                feed.insertAdjacentHTML('beforeend', '<p class="text-muted text-center" style="font-size:0.85rem; margin-top:20px;">Sin respuestas aún.</p>'); return;
            }
            [...chatData.messages].reverse().forEach(msg => renderMsg(msg, false));
        } catch (error) { feed.innerHTML = '<p class="text-muted text-center" style="color:#ef4444;">Error al cargar comentarios.</p>'; }
    },
    ...
    setupEventListeners() {
        ...
        const emojiPicker = document.getElementById('emoji-picker-container-control');

        sendBtn.addEventListener('click', () => this.sendMessage());
        chatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') this.sendMessage(); });
        emojiBtn.addEventListener('click', () => { emojiPicker.classList.toggle('hidden'); });
        document.querySelector('emoji-picker').addEventListener('emoji-click', e => { chatInput.value += e.detail.unicode; emojiPicker.classList.add('hidden'); chatInput.focus(); });
    },
    ...
    async setupPollSystem() {
        ...
        pollToggleBtn.addEventListener('click', () => {
            const isHidden = pollContent.classList.toggle('hidden');
            pollChevron.style.transform = isHidden ? 'rotate(0deg)' : 'rotate(180deg)';
        });
        ...
        this.supabase.channel('control-poll-votes')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'poll_votes', filter: `session_id=eq.${this.sessionId}` }, (p) => {
                this.pollCounts[p.new.emoji] = (this.pollCounts[p.new.emoji] || 0) + 1;
    ...
    renderPollControlUI(status, emojis) {
        ...
        const setupArea = document.getElementById('poll-setup-area'); const resultsArea = document.getElementById('poll-results-area');
        const indicator = document.getElementById('poll-status-indicator');
        const btnLaunch = document.getElementById('btn-launch-poll'); const btnClose = document.getElementById('btn-close-poll');
    ...
    updatePollCountersUI(emojis) {
        const container = document.getElementById('poll-counters');
        if (container) container.innerHTML = emojis.map(emj => `<div class="poll-counter-item"><span class="poll-counter-emoji">${emj}</span><span class="poll-counter-value">${this.pollCounts[emj] || 0}</span></div>`).join('');
    },

    generateOBSCollection(session, obsUrl) {
        if (!obsUrl) { alert("El enlace no está disponible."); return; }
        const safeTitle = (session.session_title || "Evento").replace(/[^a-z0-9]/gi, '_').toLowerCase();
        
        const obsJson = {
            "current_scene": "EPT Live", "name": `EPT_${safeTitle}`,
            "sources": [{ "name": "Cámara EPT Live", "settings": { "url": obsUrl, "width": 1920, "height": 1080, "reroute_audio": true } }]
        };
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(obsJson));
